...
{
    /**
     * Import existing static AGB sections as German translations.
     * Run manually: php artisan db:seed --class=TermsSectionSeeder
     *
     * Does not enable DYNAMIC_TERMS_ENABLED — keep that off until content is reviewed.
     */
    public function run(): void
    {
        if (TermsSection::exists()) {
            $this->command?->warn('terms_sections already has data — skipping TermsSectionSeeder.');
            return;
        }
        ...
        $this->command?->info('Seeded ' . count($sections) . ' T&C sections (DE). Review in admin, then enable DYNAMIC_TERMS_ENABLED.');
    }
    ...
    private function sections(): array
    {
        return [
            ...
            [
                'title' => 'Datenschutz',
                'content' => '<p>Die Einhaltung der datenschutzrechtlichen Regelungen (DSGVO und BDSG) nehmen wir sehr ernst. Zum Umgang mit den Nutzer- und Anbieterdaten verweisen wir auf unsere Datenschutzerklärung unter https://catchaguide.de/data-protection.</p>',
            ],
            [
