@php
    $copy = $copyNamespace ?? 'emails.product_report_admin';
    $adminListUrl = route('admin.product-reports.index');
    $sourceLabel = $report->getSourceLabel();
    $sourceFrontUrl = $report->getSourceFrontUrl();
    $sourceAdminUrl = $report->getSourceAdminUrl();
@endphp
<!DOCTYPE html>
            ...
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 24px rgba(0,0,0,0.08); overflow: hidden;">
                                ...
                                <td style="padding: 12px 14px;">
                                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="font-size: 13px; color: #334155;">
                                        <tr>
                                            ...
                                            <td style="padding: 5px 0; color: #0f172a;">{{ $report->name }}</td>
                                        </tr>
                                        <tr>
                                            <td style="padding: 5px 12px 5px 0; font-weight: 600; color: #64748b; vertical-align: top;">{{ __($copy . '.email') }}</td>
                                            <td style="padding: 5px 0;"><a href="mailto:{{ $report->email }}" style="color: #e8604c; text-decoration: none;">{{ $report->email }}</a></td>
                                        </tr>
                                        @if($report->phone)
                                        <tr>
                                            <td style="padding: 5px 12px 5px 0; font-weight: 600; color: #64748b; vertical-align: top;">{{ __($copy . '.phone') }}</td>
                                            <td style="padding: 5px 0; color: #0f172a;">{{ $report->phone }}</td>
                                        </tr>
                                        @endif
                                    </table>
                                </td>
                                ...
                                <td style="padding: 12px 14px;">
                                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="font-size: 13px; color: #334155;">
                                        <tr>
                                            ...
                                            <td style="padding: 5px 0; color: #0f172a; font-weight: 600;">{{ $report->reason_label }}</td>
                                        </tr>
                                        <tr>
                                        ...
                                        @if($sourceLabel)
                                        <tr>
                                            <td style="padding: 5px 12px 5px 0; font-weight: 600; color: #64748b; vertical-align: top;">{{ __($copy . '.source') }}</td>
                                            <td style="padding: 5px 0; color: #0f172a;">{{ $sourceLabel }}</td>
                                        </tr>
                                        @endif
                                        @if($sourceFrontUrl)
                                        <tr>
                                            <td style="padding: 5px 12px 5px 0; font-weight: 600; color: #64748b; vertical-align: top;">{{ __($copy . '.listing_url') }}</td>
                                            <td style="padding: 5px 0;"><a href="{{ $sourceFrontUrl }}" target="_blank" style="color: #e8604c; text-decoration: none; word-break: break-all;">{{ $sourceFrontUrl }}</a></td>
                                        </tr>
                                        @endif
                                        @if($sourceAdminUrl)
                                        <tr>
                                            <td style="padding: 5px 12px 5px 0; font-weight: 600; color: #64748b; vertical-align: top;">{{ __($copy . '.admin_url') }}</td>
                                            <td style="padding: 5px 0;"><a href="{{ $sourceAdminUrl }}" target="_blank" style="color: #e8604c; text-decoration: none;">{{ __($copy . '.open_admin') }}</a></td>
                                        </tr>
                                        @endif
                                    </table>
                                </td>
                ...
                <tr>
                    <td style="padding: 0 20px 18px; text-align: center;">
                        <a href="{{ $adminListUrl }}" target="_blank" style="display: inline-block; background-color: #e8604c; color: #ffffff !important; padding: 10px 24px; font-size: 13px; font-weight: 600; text-decoration: none; border-radius: 6px; margin-right: 8px;">{{ __($copy . '.view_reports') }}</a>
                ...
                <tr>
                    <td style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); padding: 20px 24px; color: #ffffff; text-align: center;">
                        <p style="margin: 0; color: #9ca3af; font-size: 12px;">© {{ date('Y') }} {{ config('app.name') }}</p>
                    </td>
                </tr>
            </table>
        </td>
