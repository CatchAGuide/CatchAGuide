...
@section('share_tags')
    <meta property="og:title" content="{{$row_data->language->title}}" />
            ...
            <div class="page-header__bottom breadcrumb-container guiding">
                <div class="page-header__bottom-inner">
                    <ul class="thm-breadcrumb list-unstyled">
                        ...
                        <li><a href="{{ route('vacations.index') }}">{{ translate('Target Fish')}}</a></li>
                        <li><span><i class="fas fa-solid fa-chevron-right"></i></span></li>
                        <li class="active">{{ $row_data->language->title }}</li>
                    </ul>
                </div>
            </div>
        </section>
        ...
        <div class="row">
            <div class="col-12">
                <div id="page-main-intro" class="mb-3">
                    <div class="page-main-intro-text mb-1">{!! nl2br($row_data->language->introduction) !!}</div>
                    <p class="see-more text-center"><a href="#" class="btn btn-primary btn-sm read-more-btn">@lang('vacations.read_more')</a></p>
                </div>
                <h5 class="mb-2">{{ $row_data->language->title }}</h5>
                <div class="row mb-5">
                            ...
                            <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                                ...
                                <a class="btn border-start cag-btn-inverted" data-bs-toggle="offcanvas" data-bs-target="#offcanvasBottomSearch" aria-controls="offcanvasBottomSearch" href="javascript:void(0)" style="border-left: 1px solid #ccc!important; z-index: 2; width:30%;">
                                    ...
                                    @if($category_total > 0)
                                        ...
                                        <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" id="guiding-filter-counter">{{ $guides->count() }}</span>
                                        @endif
                                    @endif
                                </a>
                            ...
                            <div class="tours-list__right col-md-12">
                                <div class="row p-2 border shadow-sm bg-white rounded">
                                    <div class="col-12 col-sm-12 col-md-4 col-lg-4 col-xl-4 col-xxl-4 mt-1 p-0">
                                        <div id="carouselExampleControls-{{$guide->id}}" class="carousel slide" data-bs-ride="carousel" data-bs-interval="false">
                                            <div class="carousel-inner">
                                                @if(count(get_galleries_image_link($guide, 1)))
                                                    @foreach(get_galleries_image_link($guide, 1) as $index => $gallery_image_link)
                                            ...
                                            @if(count(get_galleries_image_link($guide, 1)) > 1)
                                                <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleControls-{{$guide->id}}" data-bs-slide="prev">
                                                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                                    <span class="visually-hidden">Previous</span>
                                                </button>
                                                <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleControls-{{$guide->id}}" data-bs-slide="next">
                                    ...
                                    <a href="{{ route('vacations.show', [$guide->id, $guide->slug]) }}" 
                                       onclick="event.preventDefault(); 
                                                document.getElementById('store-destination-{{ $guide->id }}').submit();">
                                        <div class="guidings-item">
                                            <div class="guidings-item-title">
                                                @if(!$agent->ismobile())
                                                <h5 class="fw-bolder text-truncate">{{translate($guide->title)}}</h5>
                                                @endif
                                                @if($agent->ismobile())
                                                    <h5 class="fw-bolder text-truncate">{{ \Str::limit(translate($guide->title), 65) }}</h5>
                                                @endif
                                                <span class=""><i class="fas fa-map-marker-alt me-2"></i>{{ $guide->location }} </span>                                      
                                            </div>
                                            <div class="inclusions-price">
                                            <div class="guiding-item-price">
                                                <h5 class="mr-1 fw-bold text-end"><span class="p-1">@lang('message.from') {{$guide->getLowestPrice()}}€ p.P.</span></h5>
                                        ...
                                        <div class="vacations-item-row">
                                            <div class="vacations-item-row-top">
                                                ...
                                                <div class="vacations-info-container"> 
                                                    ...
                                                    {{-- <span class="text-regular">{{ count($guide->boats) > 0 ? translate('Available') : translate('Unavailable') }}</span> --}}
                                                </div>
                                                <div class="vacations-info-container"> 
                                                    <span class="fw-bold">{{translate('Distance to the water')}}:</span>
                                                    <div class="">
                                                        {{ $guide->water_distance }}
                                                    </div>
                                                </div>
                                            <div class="vacations-info-container"> 
                                                ...
                                                <div class="d-flex">
                                                    @php
                                                        $target_fish = json_decode($guide->target_fish);
                                                    @endphp
                                    ...
                                    <form id="store-destination-{{ $guide->id }}" 
                                          action="{{ route('vacations.show', [$guide->id, $guide->slug]) }}" 
...
<script type="module">
    import { MarkerClusterer } from "https://cdn.skypack.dev/@googlemaps/markerclusterer@2.3.1";
        ...
        @php
            $lat = isset($guides[0]) ? $guides[0]->latitude : 51.165691;
            $lng = isset($guides[0]) ? $guides[0]->longitude : 10.451526;
        @endphp
