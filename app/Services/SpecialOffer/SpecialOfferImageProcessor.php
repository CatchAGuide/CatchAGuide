...
class SpecialOfferImageProcessor
{
    /**
     * Process image uploads with isolated storage strategy
     */
    public function processImageUploads(Request $request, string $slug, ?int $specialOfferId = null): ?array
    {
        $galleryImages = [];
        $thumbnailPath = '';
        ...
        Log::info('SpecialOffer Image Upload Debug:', [
        ...
        if ($request->input('is_update') == '1' && $specialOfferId) {
            ...
            Log::info('SpecialOffer Update - Existing Images:', [
        ...
        if ($request->hasFile('title_image')) {
            ...
            $tempSlug = $slug ?: 'temp-special-offer';
            $processedFilenames = [];

            foreach($request->file('title_image') as $index => $image) {
                try {
                    $originalFilename = $image->getClientOriginalName();
                    $filename = 'special-offers-images/' . $originalFilename;
                    ...
                    if (in_array($originalFilename, $imageList) || in_array($filename, $imageList) || in_array('/' . $filename, $imageList)) {
                        ...
                        // Use isolated directory structure: special-offers/{id}/gallery/
                        $directory = $specialOfferId ? "special-offers/{$specialOfferId}/gallery" : "special-offers/temp/gallery";
                        
                        $webp_path = media_upload($image, $directory, $filename, 75, $specialOfferId);
                        $galleryImages[] = $webp_path;
                        $processedFilenames[] = $originalFilename;
                        
                        Log::info('SpecialOffer Image Stored:', [
                            'path' => $webp_path, 
                            'size' => $image->getSize(),
                    ...
                    Log::error('Error storing special offer image:', [
                        'error' => $e->getMessage(),
            ...
            Log::info('SpecialOfferImageProcessor::processImageUploads - No title_image files found');
        }
        ...
        if ($request->hasFile('cropped_image')) {
            foreach($request->file('cropped_image') as $image) {
                try {
                    $timestamp = time();
                    $filename = ($slug ?: 'temp-special-offer') . "-cropped-" . count($galleryImages) . "-" . $timestamp;
                    ...
                    Log::info('SpecialOffer Cropped Image Stored:', [
                        'path' => $webp_path, 
                        'size' => $image->getSize()
                    ]);
                } catch (\Exception $e) {
                    Log::error('Error storing cropped special offer image:', [
                        'error' => $e->getMessage(),
                        'filename' => $image->getClientOriginalName()
                    ]);
                }
        ...
        Log::info('Final SpecialOffer Gallery Images Array:', [
            'gallery_images' => $galleryImages,
    ...
    /**
     * Move images from temp directory to final directory after special offer creation
     */
    public function moveImagesToFinalDirectory(int $specialOfferId, string $slug, array $imageData): array
    {
        $tempDirectory = "special-offers/temp/gallery";
        $finalDirectory = "special-offers/{$specialOfferId}/gallery";
    ...
    private function deleteImage(string $imagePath): void
    {
        try {
            ...
            Log::info('SpecialOffer Image Deleted:', ['path' => $imagePath]);
        } catch (\Exception $e) {
            Log::error('Error deleting special offer image:', [
                'error' => $e->getMessage(),
                'path' => $imagePath
            ]);
        }
